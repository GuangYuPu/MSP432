...
void TimA1_PWM_Init(uint16_t ccr0, uint16_t psc)
{
    ...
    Timer_A_PWMConfig TimA11_PWMConfig;
    /*定时器PWM初始化*/
    TimA11_PWMConfig.clockSource = TIMER_A_CLOCKSOURCE_SMCLK;             //时钟源
    TimA11_PWMConfig.clockSourceDivider = psc;                            //时钟分频 范围1-64
    TimA11_PWMConfig.timerPeriod = ccr0;                                  //自动重装载值（ARR）
    TimA11_PWMConfig.compareRegister = TIMER_A_CAPTURECOMPARE_REGISTER_1; //通道一 （引脚定义）
    TimA11_PWMConfig.compareOutputMode = TIMER_A_OUTPUTMODE_TOGGLE_SET;   //输出模式
    TimA11_PWMConfig.dutyCycle = (uint16_t)(float)ccr0/2;                                    //这里是改变占空比的地方 默认50%

    MAP_Timer_A_generatePWM(TIMER_A1_BASE, &TimA11_PWMConfig); /* 初始化比较寄存器以产生 PWM1 */

    /*pwm 输出通道二：p7.6*/
    /*初始化引脚*/
    MAP_GPIO_setAsPeripheralModuleFunctionOutputPin(GPIO_PORT_P7, GPIO_PIN6, GPIO_PRIMARY_MODULE_FUNCTION);

    Timer_A_PWMConfig TimA12_PWMConfig;
    /*定时器PWM初始化*/
    TimA12_PWMConfig.clockSource = TIMER_A_CLOCKSOURCE_SMCLK;             //时钟源
    TimA12_PWMConfig.clockSourceDivider = psc;                            //时钟分频 范围1-64
    TimA12_PWMConfig.timerPeriod = ccr0;                                  //自动重装载值（ARR）
    TimA12_PWMConfig.compareRegister = TIMER_A_CAPTURECOMPARE_REGISTER_2; //通道二 （引脚定义）
    TimA12_PWMConfig.compareOutputMode = TIMER_A_OUTPUTMODE_TOGGLE_SET;   //输出模式
    TimA12_PWMConfig.dutyCycle = (uint16_t)(float)ccr0/3;                                    //这里是改变占空比的地方 默认33%

    MAP_Timer_A_generatePWM(TIMER_A1_BASE, &TimA12_PWMConfig); /* 初始化比较寄存器以产生 PWM2 */

    /*pwm 输出通道三：p7.5*/
    /*初始化引脚*/
    MAP_GPIO_setAsPeripheralModuleFunctionOutputPin(GPIO_PORT_P7, GPIO_PIN5, GPIO_PRIMARY_MODULE_FUNCTION);

    Timer_A_PWMConfig TimA13_PWMConfig;
    /*定时器PWM初始化*/
    TimA13_PWMConfig.clockSource = TIMER_A_CLOCKSOURCE_SMCLK;             //时钟源
    TimA13_PWMConfig.clockSourceDivider = psc;                            //时钟分频 范围1-64
    TimA13_PWMConfig.timerPeriod = ccr0;                                  //自动重装载值（ARR）
    TimA13_PWMConfig.compareRegister = TIMER_A_CAPTURECOMPARE_REGISTER_3; //通道三 （引脚定义）
    TimA13_PWMConfig.compareOutputMode = TIMER_A_OUTPUTMODE_TOGGLE_SET;   //输出模式
    TimA13_PWMConfig.dutyCycle = (uint16_t)(float)ccr0*3/4;                                    //这里是改变占空比的地方 默认75%

    MAP_Timer_A_generatePWM(TIMER_A1_BASE, &TimA13_PWMConfig); /* 初始化比较寄存器以产生 PWM3 */
}
/*********************************************************************************************************/
...
// 6.编写TIMA ISR
__weak void TA0_0_IRQHandler(void)
{
    MAP_Timer_A_clearCaptureCompareInterrupt(TIMER_A0_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_0);
